...
- **`tracely replay <agent>`** — re-run the agent **on each promoted case's recorded input** (fetched from `GET /api/gate/suite`), then gate. `--entrypoint module:func` calls a Python function per case; `--cmd "…"` runs a shell command per case (gets `TRACELY_INPUT`) that emits its own trace. Strict recorded replay by default (a call the recording lacks fails the case rather than running live); `--live` makes real tool/LLM calls, `--lenient` restores the legacy fall-through (reported as such). `GET /api/gate/suite` returns each case's `fixtures` bundle, or `fixtures: null` plus a `fixture_error` when the recording is missing or unreadable.
