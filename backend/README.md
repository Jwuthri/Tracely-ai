...
| A stable re-record replaces only its own groups | One eval trace holds **every** column graded at that (subject, level) — its id is derived from them. A per-column re-run records one group, so wiping the whole trace erased the other columns' prompts and only the last column run stayed readable (`deletes.delete_trace(..., step_names)`). |

That last property is what `GET /api/evaluations/prompt` depends on: the score row stores no pointer to its recording, so the reader re-derives the trace id (`introspection.stable_trace_id`) and filters it to one column — and, at step level, to the one span the cell is about. ponytail: a partial re-record's root span summarizes only the column that ran, so the eval row's collapsed output can understate what the trace holds.
...
| `GET /api/evaluations/prompt` | `evaluations.py` | the judge's prompt + reply behind one score cell, read back from that grade's eval recording (`subject`+`level`+`name`, `step` for a step-level column). |
